...
const AccountSection = ({
  ...
  const handleDeleteAccountAndReset = async () => {
    ...
    try {
      setDeleteMessage(
        "Please re-authenticate with Google to confirm account deletion.",
      );
      await reauthenticateWithPopup(currentUser, provider);
    } catch (error) {
      console.error("Re-authentication for deletion failed:", error);
      setDeleteMessage(
        `Re-authentication failed. Deletion cancelled. Error: ${error.code}`,
      );
      setTimeout(() => {
    ...
    try {
      ...
      setDeleteMessage(
        `Deletion failed: ${error.message}. Please sign out and sign back in.`,
      );
  ...
  return (
    <div className="mb-8 p-4 bg-gray-800 border border-purple-700 rounded-lg shadow-sm">
      ...
      {user && !user.isAnonymous && (
        <div className="text-left mb-4 bg-gray-700 p-3 rounded-md border border-green-500">
          <p className="text-sm text-green-400 font-medium">
            ...
            <span className="block font-mono text-green-100 mt-1">
              {googleEmail}
            </span>
          </p>
        </div>
      ...
      {!savedSubmissivesName && isAuthReady && (
        <div className="mb-4">
          <label
            htmlFor="settingsSubmissivesName"
            className="block text-sm font-medium text-purple-300 mb-1 text-left"
          >
      ...
      {nameMessage && (
        <p
          className={`text-xs mt-2 mb-3 text-left ${nameMessage.includes("successfully") || nameMessage.includes("set") ? "text-green-400" : "text-yellow-400"}`}
        >
          {nameMessage}
        </p>
      )}
      ...
      {(!user || user.isAnonymous) && (
        <div>
          <h4 className="text-lg font-medium text-purple-200 mb-2 text-left">
            Account Sync & Backup
          </h4>
          <p className="text-xs text-purple-400 mb-3 text-left">
            Your data is currently only stored on this device. Sign in with
            Google to sync your data across devices and prevent data loss.
          </p>
          ...
          {showUserIdInSettings && userId && (
            <div className="p-3 bg-gray-700 rounded-md text-left">
              <p className="text-sm text-purple-300">
                ...
                <span className="font-mono text-purple-100 select-all">
                  {userId}
                </span>
              </p>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-red-700 rounded-lg shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-sm text-gray-300 mb-4">
              This will permanently delete your Google account link AND all
              associated tracker data. This action cannot be undone.
            </p>
            <p className="text-sm text-yellow-300 mb-4">
              You will be asked to sign in with Google again to confirm this is
              your account.
            </p>
            {deleteMessage && (
              <p className="text-sm font-bold text-center text-yellow-300 my-3">
                {deleteMessage}
              </p>
            )}
