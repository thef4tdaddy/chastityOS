...
const CurrentStatusSection = ({
  ...
  <>
    ...
    <div className="mb-1">
      <strong>Cage Status:</strong>{" "}
      {isCageOn ? (isPaused ? "ON (Paused)" : "ON") : "OFF"}
    </div>
    {isCageOn && cageOnTime && (
      <div className="mb-1">
        <strong>Current Cage On Since:</strong> {formatTime(cageOnTime, true)}
      </div>
    )}
    <div
      className={`p-2 my-1 rounded ${isCageOn ? (isPaused ? "bg-yellow-500/10" : "bg-green-500/10") : ""}`}
    >
      <strong>Effective Time This Session:</strong>
      <span
        className={`ml-2 font-semibold ${isCageOn ? (isPaused ? "text-yellow-400" : "text-green-400") : "text-purple-200"}`}
      >
      ...
      {isCageOn && accumulatedPauseTimeThisSession > 0 && (
        <span className="text-xs text-yellow-300 block mt-1">
          (Total paused this session:{" "}
          {formatElapsedTime(accumulatedPauseTimeThisSession)})
        </span>
      )}
    ...
    <div
      className={`p-2 my-1 rounded ${!isCageOn && timeCageOff > 0 ? "bg-red-500/10" : ""}`}
    >
      <strong>Current Session Cage Off:</strong>{" "}
      <span
        className={`ml-2 font-semibold ${!isCageOn && timeCageOff > 0 ? "text-red-400" : "text-purple-200"}`}
      >
        {formatElapsedTime(timeCageOff)}
      </span>
    </div>
  </>
);
