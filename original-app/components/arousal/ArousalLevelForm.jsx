...
const ArousalLevelForm = ({
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="w-full bg-accent hover:bg-accent-dark text-white font-semibold py-2 px-4 rounded-md shadow-sm transition duration-300"
      >
        Log Arousal Level
      </button>
      {arousalMessage && (
        <p className="text-sm mt-2 text-green-400">{arousalMessage}</p>
      )}
    </form>
  );
};
