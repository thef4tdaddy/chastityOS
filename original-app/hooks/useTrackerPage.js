...
export const useTrackerPage = (props) => {
  ...
  const [showEmergencyUnlockModal, setShowEmergencyUnlockModal] =
    useState(false);
  const [backupCodeInput, setBackupCodeInput] = useState("");
  const [unlockMessage, setUnlockMessage] = useState("");

  // Wrap safe initial values in useMemo to stabilize them and prevent unnecessary re-renders
  const safeTimeInChastity = useMemo(
    () => timeInChastity || 0,
    [timeInChastity],
  );
  const safeAccumulatedPauseTime = useMemo(
  ...
  useEffect(() => {
    if (
      ...
      const calculateRemaining = () => {
        const currentEffectiveChastity = Math.max(
          0,
    ...
    if (isCageOn) {
      ...
      if (safeHistory.length > 0) {
        return {
          ...
          topBoxTime: safeHistory[safeHistory.length - 1].endTime,
        };
      }
