...
export const useChastitySession = (
  ...
  const [showRestoreSessionPrompt, setShowRestoreSessionPrompt] =
    useState(false);
  const [loadedSessionData, setLoadedSessionData] = useState(null);
  const [hasSessionEverBeenActive, setHasSessionEverBeenActive] =
    useState(false);
  ...
  const [accumulatedPauseTimeThisSession, setAccumulatedPauseTimeThisSession] =
    useState(0);
  ...
  const saveDataToFirestore = useCallback(
    async (dataToSave) => {
      if (!isAuthReady || !userId) {
        console.warn(
          "Attempted to save data before authentication was ready or user ID was available.",
        );
        return;
      }
      ...
      if (!docRef) {
        console.error(
          "Firestore document reference is null, cannot save data.",
        );
        return;
      }
      try {
  ...
  const applyRestoredData = useCallback((data) => {
    if (!data || typeof data !== "object") {
      console.warn(
        "⚠️ Skipping applyRestoredData: invalid or empty data",
        data,
      );
      return;
    }
    ...
    const lPauseEndTime = data.lastPauseEndTime?.toDate
      ? data.lastPauseEndTime.toDate()
      : null;
    setLastPauseEndTime(
      lPauseEndTime && !isNaN(lPauseEndTime.getTime()) ? lPauseEndTime : null,
    );
    const loadedCageOn = data.isCageOn || false;
    const loadedCageOnTime = data.cageOnTime?.toDate
      ? data.cageOnTime.toDate()
      : null;
    const loadedPauseStart = data.pauseStartTime?.toDate
      ? data.pauseStartTime.toDate()
      : null;
    const loadedCageOffStart = data.cageOffStartTime?.toDate
      ? data.cageOffStartTime.toDate()
      : null;
    setIsCageOn(loadedCageOn);
    setCageOnTime(
      loadedCageOn && loadedCageOnTime && !isNaN(loadedCageOnTime.getTime())
        ? loadedCageOnTime
        : null,
    );
    if (loadedCageOn && loadedCageOnTime) {
      ...
      const elapsed = Math.floor(
        (now.getTime() - loadedCageOnTime.getTime()) / 1000,
      );
    ...
    setPauseStartTime(
      loadedCageOn &&
        data.isPaused &&
        loadedPauseStart &&
        !isNaN(loadedPauseStart.getTime())
        ? loadedPauseStart
        : null,
    );
    setAccumulatedPauseTimeThisSession(
      loadedCageOn ? data.accumulatedPauseTimeThisSession || 0 : 0,
    );
    setCageOffStartTime(
      !loadedCageOn &&
        loadedCageOffStart &&
        !isNaN(loadedCageOffStart.getTime())
        ? loadedCageOffStart
        : null,
    );
    if (!loadedCageOn && loadedCageOffStart) {
      const now = new Date();
      const elapsedOff = Math.floor(
        (now.getTime() - loadedCageOffStart.getTime()) / 1000,
      );
    ...
    setHasSessionEverBeenActive(
      data.hasSessionEverBeenActive !== undefined
        ? data.hasSessionEverBeenActive
        : true,
    );

    // --- Load the Keyholder Duration ---
    setRequiredKeyholderDurationSeconds(
      data.requiredKeyholderDurationSeconds || 0,
    );
  ...
  const handleConfirmRemoval = useCallback(async () => {
    ...
    if (isPaused && pauseStartTime) {
      finalAccumulatedPauseTime += Math.max(
        0,
        Math.floor((tempEndTime.getTime() - pauseStartTime.getTime()) / 1000),
      );
    }
    const rawDurationSeconds = Math.max(
      0,
      Math.floor((tempEndTime.getTime() - tempStartTime.getTime()) / 1000),
    );
  ...
  const handleUpdateCurrentCageOnTime = useCallback(async () => {
    ...
    if (
      !isCageOn ||
      !(cageOnTime instanceof Date) ||
      isNaN(cageOnTime.getTime())
    ) {
      setEditSessionMessage("No valid active session to edit.");
      Sentry.captureMessage(
        "handleUpdateCurrentCageOnTime called with invalid cageOnTime",
        { extra: { cageOnTime } },
      );
    ...
    const eventsColRef =
      typeof getEventsCollectionRef === "function"
    ...
    setTimeInChastity(
      Math.max(
        0,
        Math.floor((new Date().getTime() - newTime.getTime()) / 1000),
      ),
    );
    // Save to Firestore
  ...
  const handleConfirmPause = useCallback(async () => {
    const now = new Date();
    const finalReason =
      pauseReason === "Other" ? pauseCustomReason || "Other" : pauseReason;
    setIsPaused(true);
    setPauseStartTime(now);
    const updatedPauseEvents = [
      ...currentSessionPauseEvents,
      { startTime: now, reason: finalReason },
    ];
  ...
  const handleResumeSession = useCallback(async () => {
    ...
    const updatedPauseEvents = currentSessionPauseEvents.map((e, i) =>
      i === currentSessionPauseEvents.length - 1
  ...
  const handleEndChastityNow = useCallback(
    async (reason = "Session ended programmatically.") => {
      ...
      if (isPaused && pauseStartTime) {
        finalAccumulatedPauseTime += Math.max(
          0,
          Math.floor((endTime.getTime() - pauseStartTime.getTime()) / 1000),
        );
      }
      const rawDurationSeconds = Math.max(
        0,
        Math.floor((endTime.getTime() - cageOnTime.getTime()) / 1000),
      );
      const newHistoryEntry = {
        id: crypto.randomUUID(),
        periodNumber: chastityHistory.length + 1,
  ...
  const handleRestoreUserIdInputChange = (e) =>
    setRestoreUserIdInput(e.target.value);
  ...
  useEffect(() => {
    ...
    if (isCageOn && !isPaused && cageOnTime) {
      timerInChastityRef.current = setInterval(() => {
        const now = new Date();
        const elapsed = Math.floor(
          (now.getTime() - cageOnTime.getTime()) / 1000,
        );
        setTimeInChastity(elapsed);
      }, 1000);
      ...
      timerCageOffRef.current = setInterval(() => {
        const now = new Date();
        const elapsed = Math.floor(
          (now.getTime() - cageOffStartTime.getTime()) / 1000,
        );
