...
export function useReleaseRequests(userId, isAuthReady) {
          ...
          return {
            ...
            submittedAt:
              item.submittedAt && typeof item.submittedAt.toDate === "function"
                ...
                : null,
            deniedAt:
              item.deniedAt && typeof item.deniedAt.toDate === "function"
                ? item.deniedAt.toDate()
                : null,
            grantedAt:
              item.grantedAt && typeof item.grantedAt.toDate === "function"
  ...
  const updateReleaseRequest = useCallback(
    async (id, updates) => {
      const colRef = getCollectionRef();
      if (!colRef) return;
      try {
  ...
  const deleteReleaseRequest = useCallback(
    async (id) => {
      const colRef = getCollectionRef();
      if (!colRef) return;
      try {
