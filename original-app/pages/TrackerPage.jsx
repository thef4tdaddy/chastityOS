...
const TrackerPage = (props) => {
  ...
  const hasPendingReleaseRequest = releaseRequests.some(
    (r) => r.status === "pending",
  );
  const lastDeniedRequest = releaseRequests
    .filter((r) => r.status === "denied" && r.deniedAt)
    .sort((a, b) => b.deniedAt - a.deniedAt)[0];
  const denialCooldownActive =
    lastDeniedRequest &&
    Date.now() - lastDeniedRequest.deniedAt.getTime() < 4 * 3600 * 1000;
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-700 p-6 md:p-8 rounded-xl shadow-lg text-center w-full max-w-md text-gray-50 border border-blue-500">
            ...
            <p className="text-sm text-gray-300 mb-2">
              An active chastity session was found:
            </p>
      ...
      {isCageOn &&
        props.goalDurationSeconds > 0 &&
        remainingGoalTime !== null && (
          <div
            className={`mb-4 p-3 rounded-lg shadow-sm text-center border ${remainingGoalTime <= 0 ? "bg-green-700 border-green-500" : "bg-blue-800 border-blue-600"}`}
          >
            <p
              className={`text-lg font-semibold ${remainingGoalTime <= 0 ? "text-green-200" : "text-blue-200"}`}
            >
              {remainingGoalTime <= 0
                ? "Goal Reached!"
      ...
      {keyholderName && requiredKeyholderDurationSeconds > 0 && (
        <div
          className={`mb-4 p-3 rounded-lg shadow-sm text-center border ${
            isCageOn &&
            effectiveTimeInChastityForGoal >= requiredKeyholderDurationSeconds
        ...
        >
          ...
          >
            ...
            to be in chastity for{" "}
            {formatDaysOnly(requiredKeyholderDurationSeconds)}
          </p>
          {isCageOn &&
            effectiveTimeInChastityForGoal <
              requiredKeyholderDurationSeconds && (
              <p className="text-lg font-bold text-purple-100">
                Time Left in required chastity:{" "}
                {formatElapsedTime(
                  requiredKeyholderDurationSeconds -
                    effectiveTimeInChastityForGoal,
                )}
      ...
      {denialCooldownActive && lastDeniedRequest && (
        <div className="mb-4 p-2 rounded-md text-center bg-red-700/30 border border-red-600">
          <p className="text-sm text-red-300">
            Beg for Release denied by{" "}
            {lastDeniedRequest.handledBy || keyholderName} at{" "}
            {lastDeniedRequest.deniedAt.toLocaleString()}
          </p>
        </div>
      )}
      ...
      <div className="space-y-4 mb-6 md:mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div
            className={`p-3 md:p-4 rounded-lg shadow-sm transition-colors duration-300 border ${isCageOn ? (isPaused ? "bg-yellow-500/20 border-yellow-600" : "bg-green-500/20 border-green-600") : "tracker-box"}`}
          >
          ...
          <div
            className={`p-3 md:p-4 rounded-lg shadow-sm transition-colors duration-300 border ${!isCageOn && timeCageOff > 0 ? "bg-red-500/20 border-red-600" : "tracker-box"}`}
          >
            <p className="tracker-label text-sm md:text-lg">
      ...
      {showReasonModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 p-6 md:p-8 rounded-xl shadow-lg text-center w-full max-w-md text-gray-50 border border-purple-700">
            <h3 className="text-lg md:text-xl font-bold mb-4 text-purple-300">
              Reason for Cage Removal:
            </h3>
            ...
            {reasonForRemoval === "Other" && (
              <input
                type="text"
                value={removalCustomReason}
                onChange={(e) => setRemovalCustomReason(e.target.value)}
                placeholder="Enter other reason"
                className="w-full p-2 mb-4 rounded-lg border border-purple-600 bg-gray-900 text-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            )}
            <div className="flex flex-col sm:flex-row justify-around space-y-3 sm:space-y-0 sm:space-x-4">
              <button
                ...
                className="w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg transition"
              >
                Confirm Removal
              </button>
              <button
          ...
          <div className="bg-gray-800 p-6 md:p-8 rounded-xl shadow-lg text-center w-full max-w-md text-gray-50 border border-yellow-700">
            ...
            {pauseReason === "Other" && (
              <input
                ...
                placeholder="Enter other reason"
                className="w-full p-2 mb-4 rounded-lg border border-yellow-600 bg-gray-900 text-gray-50 focus:outline-none focus:ring-2 focus:ring-yellow-500"
              />
            )}
            <div className="flex flex-col sm:flex-row justify-around space-y-3 sm:space-y-0 sm:space-x-4">
              <button
                ...
                className="w-full sm:w-auto bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-lg transition"
              >
                Confirm Pause
              </button>
              <button
                type="button"
                onClick={handleCancelPauseModal}
                className="w-full sm:w-auto bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg transition"
              >
                Cancel
              </button>
