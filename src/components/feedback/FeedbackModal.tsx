...
import { FaBug, FaLightbulb, FaComment, FaTimes, FaPaperPlane } from 'react-icons/fa';
...
const FeedbackModal: React.FC<FeedbackModalProps> = ({ type, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const feedbackData: FeedbackData = {
        type,
        ...formData,
        systemInfo: formData.includeSystemInfo ? await collectSystemInfo() : null,
        timestamp: new Date(),
  ...
  const getModalIcon = () => {
    switch (type) {
      ...
      case 'feature': return <FaLightbulb className="text-blue-400" />;
      case 'general': return <FaComment className="text-green-400" />;
      default: return <FaComment className="text-green-400" />;
    }
  };
  ...
  const getTitlePlaceholder = () => {
    switch (type) {
      ...
      case 'feature': return 'Short title for your feature request';
      case 'general': return 'Summary of your feedback';
      default: return 'Brief title';
    }
  };

  const getDescriptionPlaceholder = () => {
    switch (type) {
      case 'bug': return 'Describe what happened and what you expected to happen';
      case 'feature': return 'Describe your feature idea and how it would help';
      case 'general': return 'Share your thoughts and suggestions';
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[70vh] space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              {type === 'bug' ? 'Bug Summary' : type === 'feature' ? 'Feature Title' : 'Feedback Title'}
            </label>
            <input
              type="text"
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium text-gray-300">Include System Information</div>
              <div className="text-xs text-gray-500">Helps us debug technical issues</div>
            </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              <FaPaperPlane />
              {isSubmitting ? 'Submitting...' : `Submit ${type === 'bug' ? 'Bug Report' : type === 'feature' ? 'Feature Request' : 'Feedback'}`}
            </button>
            <button
              type="button"
              onClick={onClose}
