      ...
      <div className="settings-section">
        ...
        <div className="text-center p-4 bg-gray-900/50 rounded-lg border border-yellow-700">
          ...
          <p className="text-md text-purple-200 mt-2">
            Personal goals cannot be set or modified while a Keyholder lock is
            active.
          </p>
        </div>
      </div>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-gray-800 p-6 md:p-8 rounded-xl shadow-lg text-center w-full max-w-lg text-gray-50 border-2 border-yellow-500">
          ...
          <p className="text-md text-purple-200 mb-4">
            Save this one-time **Backup Code**. It is the ONLY way to unlock
            early.
          </p>
          ...
          <p className="text-xs text-yellow-400 mb-6">
            Store it somewhere safe. If you lose this code, you cannot end your
            session until the goal is met.
          </p>
          <button
            type="button"
          ...
          {isGoalCompleted ? (
            <div>
              ...
              {revealedSelfLockCode ? (
                <>
                  <p className="text-md text-purple-200 mt-2">
                    Here is the combination you saved:
                  </p>
                  <div className="bg-gray-900 p-4 rounded-lg my-4">
                    <p className="text-3xl font-mono tracking-widest text-white">
                      {revealedSelfLockCode}
                ...
                <p className="text-md text-purple-200 mt-2">
                  You have reached the end of your goal.
                </p>
              )}
              <button
                onClick={handleClearPersonalGoal}
            ...
            <div>
              ...
              <p className="text-lg font-bold text-white">
                Ends on:{" "}
                {goalEndDate
                  ? new Date(goalEndDate).toLocaleString()
                  : "Calculating..."}
              </p>
              {remainingTime !== null && (
                <p className="text-sm text-gray-400">
                  ({formatElapsedTime(Math.round(remainingTime / 1000))}{" "}
                  remaining)
                </p>
              )}
              {!isHardcoreGoal && (
          ...
          <div className="mt-4 p-3 bg-gray-800/60 border border-gray-700 rounded-lg">
            <label className="flex items-center cursor-pointer">
              ...
              <span className="ml-3 text-sm font-medium text-purple-200">
                Hardcore Mode? (Locks Cage Off button)
              </span>
            </label>
            {isSelfLocking && (
              <div className="mt-3">
                <p className="text-xs text-yellow-400 mb-2">
                  Optionally, enter your physical lock's combination. The app
                  will save it and reveal it to you only after the goal is
                  complete.
                </p>
                <input
                  type="text"
