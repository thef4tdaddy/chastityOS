  ...
  return (
    <div className="mb-8 p-4 bg-gray-800 border border-purple-700 rounded-lg shadow-sm">
      ...
      <div className="flex flex-col space-y-3">
        ...
        <button
          type="button"
          onClick={() => handleExportClick("tracker")}
          disabled={
            !isAuthReady || (chastityHistory && chastityHistory.length === 0)
          }
        ...
        <button
          type="button"
          onClick={() => handleExportClick("eventlog")}
          disabled={
            !isAuthReady || (sexualEventsLog && sexualEventsLog.length === 0)
          }
      ...
      {eventLogMessage && !eventLogMessage.includes("restored") && (
        <p
          className={`text-xs mt-3 ${eventLogMessage.includes("successfully") || eventLogMessage.includes("exported") ? "text-green-400" : "text-yellow-400"}`}
        >
          {eventLogMessage}
        </p>
      )}
      ...
      {eventLogMessage && eventLogMessage.includes("restored") && (
        <p
          className={`text-xs mt-3 ${eventLogMessage.includes("successfully") || eventLogMessage.includes("restored") ? "text-green-400" : "text-yellow-400"}`}
        >
          {eventLogMessage}
        </p>
      )}
      ...
      <hr className="my-4 border-purple-600" />
      <div className="mb-8">
        <h4 className="text-lg font-medium text-sky-300 mb-2">
          Restore from User ID
        </h4>
        <p className="text-sm text-purple-200 mb-3">
          Enter another User ID to load their data.{" "}
          <strong className="text-yellow-400">Warning:</strong> This will
          overwrite your current data.
        </p>
        <p className="text-xs text-sky-400 mb-3">
          Note: The User ID you are restoring from must have existing data saved
        ...
        {restoreFromIdMessage && (
          <p
            className={`text-xs mt-2 ${restoreFromIdMessage.includes("successfully") || restoreFromIdMessage.includes("found") ? "text-green-400" : "text-red-500"}`}
          >
            {restoreFromIdMessage}
          </p>
        )}
      ...
      <p className="text-sm text-purple-200 mb-3">
        This action is irreversible. It will delete all data including logs and
        settings.
      </p>
      <button
        type="button"
      ...
      {nameMessage && nameMessage.includes("reset") && (
        <p
          className={`text-xs mt-2 ${nameMessage.includes("reset") ? "text-green-400" : "text-yellow-400"}`}
        >
          {nameMessage}
        </p>
      )}
      ...
      {showRestoreFromIdPrompt && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 p-6 md:p-8 rounded-xl shadow-lg text-center w-full max-w-md text-gray-50 border border-red-700">
            <h3 className="text-lg md:text-xl font-bold mb-4 text-red-400">
              Confirm Data Restore
            </h3>
            <p className="text-sm text-purple-200 mb-2">
              You are about to restore data from User ID:{" "}
              <strong className="text-sky-300">{restoreUserIdInput}</strong>.
            </p>
            <p className="text-sm text-yellow-400 font-semibold mb-6">
              This action will PERMANENTLY OVERWRITE all of your current
              ChastityOS data (history, events, settings). This cannot be
              undone.
            </p>
