...
import { Relationship, RelationshipRequest, RelationshipStatus } from "@/types/relationships";
import { 
  FaUserPlus, 
  FaUsers, 
  FaCog, 
...
const RelationshipManager: React.FC<RelationshipManagerProps> = ({ 
  ...
  const handleSendRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await sendRelationshipRequest(requestForm.email, requestForm.role, requestForm.message);
        ...
        <div className="flex items-start">
          ...
          <div className="flex-1">
            ...
            <p className="text-yellow-700 text-sm mb-3">
              We've detected existing chastity data that can be migrated to the new relationship-based system. 
              This will create a self-managed profile that you can later share with a keyholder.
            </p>
            <button
    ...
    if (relationships.length === 0) {
      return (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm">Send a request to connect with a keyholder or submissive</p>
        </div>
      );
    }
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  <h3 className="font-semibold text-gray-900">
                    Relationship with {relationship.submissiveId === relationship.keyholderId 
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Your role: {relationship.submissiveId === relationship.keyholderId 
                    ? "Both" 
                    : relationship.submissiveId === "current-user-id" ? "Submissive" : "Keyholder"}
                </p>
                {relationship.notes && (
                  <p className="text-sm text-gray-700 mt-1 italic">
                    "{relationship.notes}"
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg p-6 w-full max-w-md">
          <h3 className="text-lg font-semibold mb-4">Send Relationship Request</h3>
          ...
          <form onSubmit={handleSendRequest} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="submissive">Submissive (they'll be your keyholder)</option>
                <option value="keyholder">Keyholder (they'll be your submissive)</option>
              </select>
            </div>
