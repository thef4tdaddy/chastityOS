...
import { FaKey, FaLink, FaUser, FaUserShield, FaCopy, FaTrash, FaExclamationTriangle } from "react-icons/fa";
...
interface AccountLinkingDemoProps {
  className?: string;
  scenario?: "submissive-with-keyholder" | "submissive-no-keyholder" | "keyholder-with-submissives";
}
...
export const AccountLinkingDemo: React.FC<AccountLinkingDemoProps> = ({ 
  ...
  const activeKeyholder = scenario === "submissive-with-keyholder" ? mockActiveKeyholder : null;
  const activeInviteCodes = scenario === "submissive-no-keyholder" ? mockInviteCodes : [];
  const relationshipSummary = scenario === "submissive-with-keyholder" 
    ? mockRelationshipSummary
    : scenario === "keyholder-with-submissives"
    ? { ...mockRelationshipSummary, hasActiveKeyholder: false, hasSubmissives: true, submissiveCount: 2 }
    : { ...mockRelationshipSummary, hasActiveKeyholder: false, hasSubmissives: false, activeKeyholderCount: 0 };

  const relationships = scenario === "keyholder-with-submissives"
    ? {
        asSubmissive: [],
        asKeyholder: [
          {
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {activeKeyholder && (
        <div className="bg-gradient-to-r from-purple-900/50 to-pink-900/50 rounded-lg p-4 border border-purple-500">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              onClick={() => setShowPermissions(showPermissions === activeKeyholder.id ? null : activeKeyholder.id)}
          ...
          <div className="text-sm text-gray-300 mb-3">
            <p>Connected: {formatDistanceToNow(activeKeyholder.acceptedAt || activeKeyholder.createdAt)} ago</p>
      ...
      {activeInviteCodes.length > 0 && (
        <div className="bg-gray-800 rounded-lg p-4 border border-purple-500/30">
          <h3 className="font-semibold text-purple-300 mb-3">Active Invite Codes</h3>
          <div className="space-y-2">
            {activeInviteCodes.map((invite) => (
              <div key={invite.id} className="flex items-center justify-between p-2 bg-gray-700 rounded">
                <div>
                  <div className="font-mono text-lg text-green-400">{invite.code}</div>
              ...
              <div key={relationship.id} className="p-3 bg-gray-700 rounded border">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-sm">
                    ...
                    <div className="text-xs text-gray-400">
                      Connected: {formatDistanceToNow(relationship.acceptedAt || relationship.createdAt)} ago
                    </div>
                  </div>
                ...
                <div className="text-xs text-gray-400">
                  Permissions: {Object.values(relationship.permissions).filter(Boolean).length} of {Object.keys(relationship.permissions).length} granted
                </div>
              </div>
            ))}
          </div>
