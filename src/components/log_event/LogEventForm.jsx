    ...
    ${isNightly ? 'bg-nightly-bg border-nightly-border' : 'bg-prod-bg border-prod-border'}`}
        >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="eventDate" className={`block text-sm font-medium text-left ${isNightly ? 'text-nightly-accent' : 'text-prod-accent'}`}>Event Date:</label>
                    <input type="date" id="eventDate" value={newEventDate} onChange={e => setNewEventDate(e.target.value)} required className={`mt-1 block w-full px-3 py-2 rounded-md border bg-app-input text-app-text focus:ring-accent focus:border-accent`}/>
                </div>
                <div>
                    <label htmlFor="eventTime" className={`block text-sm font-medium text-left ${isNightly ? 'text-nightly-accent' : 'text-prod-accent'}`}>Event Time:</label>
                    <input type="time" id="eventTime" value={newEventTime} onChange={e => setNewEventTime(e.target.value)} required className={`mt-1 block w-full px-3 py-2 rounded-md border bg-app-input text-app-text focus:ring-accent focus:border-accent`}/>
                </div>
            </div>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="eventDurationHours" className={`block text-sm font-medium text-left ${isNightly ? 'text-nightly-accent' : 'text-prod-accent'}`}>Duration (Hours):</label>
                ...
                <div>
                    <label htmlFor="eventDurationMinutes" className={`block text-sm font-medium text-left ${isNightly ? 'text-nightly-accent' : 'text-prod-accent'}`}>Duration (Minutes):</label>
            ...
            <div>
                <label className={`block text-sm font-medium text-left mb-1 ${isNightly ? 'text-nightly-accent' : 'text-prod-accent'}`}>Event Type(s):</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {filteredEventTypes.map(typeDef => { // Changed to filteredEventTypes
                        ...
                        const displayLabel = type === "Orgasm (Self)" && savedSubmissivesName
                            ...
                            : type;
                        return (
                            <label key={type} className="flex items-center space-x-2 text-sm" style={{color: isNightly ? undefined : undefined}}>
            ...
            {showSelfOrgasmAmountInput && (
                 <div>
                    <label htmlFor="selfOrgasmAmount" className={`block text-sm font-medium text-left ${isNightly ? 'text-nightly-accent' : 'text-prod-accent'}`}>
            ...
            {showPartnerOrgasmAmountInput && (
                 <div>
                    <label htmlFor="partnerOrgasmAmount" className={`block text-sm font-medium text-left ${isNightly ? 'text-nightly-accent' : 'text-prod-accent'}`}>
