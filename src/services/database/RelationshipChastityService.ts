...
class RelationshipChastityService {
   ...
   */
  async getChastityData(relationshipId: string): Promise<RelationshipChastityData | null> {
    try {
  ...
  async startSession(
    ...
    try {
        ...
        events: [
          {
            ...
            initiatedBy: userId === chastityData?.keyholderId ? "keyholder" : "submissive",
      ...
      batch.set(doc(db, "chastityData", relationshipId, "sessions", sessionId), {
        ...sessionData,
      ...
      batch.update(doc(db, "chastityData", relationshipId), {
        currentSession: {
          ...
          keyholderApprovalRequired: chastityData?.settings.requireReasonForEnd || false,
        },
        updatedAt: serverTimestamp(),
      });
  ...
  async endSession(
    ...
    try {
      ...
      const sessionRef = doc(db, "chastityData", relationshipId, "sessions", sessionId);
      ...
      const totalDuration = endTime.getTime() - sessionData.startTime.toDate().getTime();

      // Add end event
      const endEvent: SessionEvent = {
        type: "end",
        timestamp: serverTimestamp() as any,
        initiatedBy: userId === await this.getKeyholderId(relationshipId) ? "keyholder" : "submissive",
        reason: endReason,
      };
  ...
  async createTask(
    ...
    try {
      ...
      const isKeyholder = userId === await this.getKeyholderId(relationshipId);
  ...
  async updateTaskStatus(
    ...
    try {
      ...
      if (status === RelationshipTaskStatus.SUBMITTED && !isKeyholder) {
        ...
        (status === RelationshipTaskStatus.APPROVED || status === RelationshipTaskStatus.REJECTED) &&
        isKeyholder
      ) {
      ...
      await updateDoc(doc(db, "chastityData", relationshipId, "tasks", taskId), updateData);
   ...
   */
  async getTasks(relationshipId: string, limitCount: number = 50): Promise<RelationshipTask[]> {
    try {
      const db = await this.ensureDb();
   ...
   */
  async getEvents(relationshipId: string, limitCount: number = 100): Promise<RelationshipEvent[]> {
    try {
      const db = await this.ensureDb();
