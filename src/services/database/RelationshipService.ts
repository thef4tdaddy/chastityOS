...
class RelationshipService {
   ...
   */
  async acceptRelationshipRequest(requestId: string, acceptingUserId: string): Promise<string> {
    try {
      const db = await this.ensureDb();
      const batch = writeBatch(db);

      // Get the request
      const requestDoc = await getDoc(doc(db, "relationshipRequests", requestId));
      ...
      const submissiveId = request.toRole === "submissive" ? request.toUserId : request.fromUserId;
      const keyholderId = request.toRole === "keyholder" ? request.toUserId : request.fromUserId;
      ...
      await this.initializeChastityData(relationshipId, submissiveId, keyholderId, batch);
   ...
   */
  async rejectRelationshipRequest(requestId: string, rejectingUserId: string): Promise<void> {
    try {
      const db = await this.ensureDb();
  ...
  async getRelationship(relationshipId: string): Promise<Relationship | null> {
    try {
      ...
      const doc = await getDoc(db.collection("relationships").doc(relationshipId));
      if (!doc.exists()) {
        return null;
      }
   ...
   * End a relationship
   */
  async endRelationship(relationshipId: string, endingUserId: string): Promise<void> {
    try {
      const db = await this.ensureDb();
      ...
      if (
        ...
        throw new Error("Only relationship participants can end the relationship");
      }
  ...
  async checkPermission(
    relationshipId: string,
    userId: string,
    action: keyof RelationshipPermissions["keyholderCanEdit"] | "pauseSession" | "emergencyUnlock",
  ): Promise<boolean> {
    try {
      const relationship = await this.getRelationship(relationshipId);
      if (!relationship) {
