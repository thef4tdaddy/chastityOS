...
export function TypedKeyholderDemo({
  ...
  const { isAdmin, permissions: adminPermissions } = useAdminSession({
    userId,
    isAuthReady,
  });

  // Keyholder session management with proper typing
  const { keyholderSession, isActive: isKeyholderActive } = useKeyholderSession(
    { userId, isAuthReady, keyholderName: "Demo Keyholder" },
  );

  // Rewards and punishments with proper typing
  const { addReward, addPunishment } = useKeyholderRewards({
    userId,
    addTask,
    saveDataToFirestore,
    requiredKeyholderDurationSeconds,
      ...
      <div className="space-y-4">
        ...
        <div className="admin-status">
          ...
          {adminPermissions && (
            <p>
              Permissions:{" "}
              {Object.keys(adminPermissions)
                .filter(
                  (key) =>
                    adminPermissions[key as keyof typeof adminPermissions],
                )
                .join(", ")}
            </p>
          )}
