...
export const useLeaderboards = (
  ...
  const [privacySettings, setPrivacySettings] = useState<LeaderboardPrivacySettings>({
  ...
  const { data: userRank, isLoading: isLoadingUserRank } = useQuery({
    ...
    queryFn: () => achievementDBService.getUserLeaderboardRank(userId!, category, period),
  ...
  const optInMutation = useMutation({
    mutationFn: async () => {
      ...
      await achievementDBService.updateLeaderboardPrivacy(userId, defaultSettings);
  ...
  const optOutMutation = useMutation({
    mutationFn: async () => {
      ...
      await achievementDBService.updateLeaderboardPrivacy(userId, optOutSettings);
    ...
    onError: (error) => {
      logger.error("Failed to opt out of leaderboards", error, "useLeaderboards");
    },
  });
  ...
  const updatePrivacyMutation = useMutation({
    ...
    onError: (error) => {
      logger.error("Failed to update privacy settings", error, "useLeaderboards");
    },
  });
   ...
   */
  const leaderboardData: LeaderboardEntry[] = rawLeaderboardData.map((entry, index) => ({
    id: entry.id,
  ...
  return {
    ...
    // Status
    isOptedIn: privacySettings.participateInGlobal || privacySettings.participateInMonthly,
