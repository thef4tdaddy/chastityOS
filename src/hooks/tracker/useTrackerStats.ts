...
import {
  ...
  determineTopBoxInfo,
} from "../../utils/tracker/calculations";
...
export const useTrackerStats = ({
  currentSession,
  topBoxLabel: legacyTopBoxLabel = "Total Locked Time",
  topBoxTime,
  ...
  const topBoxInfo = currentSession
    ? determineTopBoxInfo(currentSession)
    : { label: legacyTopBoxLabel, timestamp: "" };
  ...
  const stats = {
    topBoxLabel: topBoxInfo.label,
    topBoxTimestamp: topBoxInfo.timestamp,
