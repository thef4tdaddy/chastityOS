...
export const useGameification = (userId: string) => {
  ...
  // Get player profile
  const { data: playerProfile = DEFAULT_PLAYER_PROFILE } = useQuery<PlayerProfile>({
    queryKey: ['gamification', 'profile', userId],
    queryFn: () => {
      const stored = localStorage.getItem(STORAGE_KEYS.PLAYER_PROFILE);
      return stored ? { ...DEFAULT_PLAYER_PROFILE, ...JSON.parse(stored) } : DEFAULT_PLAYER_PROFILE;
    },
  ...
  const { data: activeChallenges = [] } = useQuery<Challenge[]>({
    ...
    queryFn: () => {
      ...
      return [...SAMPLE_CHALLENGES, ...userChallenges].filter(c => !c.isCompleted);
    },
    enabled: Boolean(userId),
    staleTime: 60 * 1000
  });
  ...
  const { data: currentSeason } = useQuery<Season | null>({
    ...
    queryFn: () => {
      return {
        ...
        description: 'Embrace the cold season with special winter-themed challenges',
        theme: 'winter',
  ...
  const completeChallengeMutation = useMutation({
    mutationFn: async (challengeId: string): Promise<ChallengeCompletion> => {
      ...
      const updatedChallenges = activeChallenges.map(c =>
        c.id === challengeId ? { ...c, isCompleted: true, progress: { ...c.progress, percentage: 100 } } : c
      );
      localStorage.setItem(STORAGE_KEYS.CHALLENGES, JSON.stringify(updatedChallenges));

      // Calculate rewards
      const experienceGained = challenge.rewards.reduce((total, reward) => 
        reward.type === 'experience' ? total + reward.value : total, 0
      );

      // Add experience
      const levelResult = await addExperienceInternal(experienceGained, ExperienceSource.CHALLENGE_COMPLETE);
      ...
      const completion: ChallengeCompletion = {
        ...
        newBadges: challenge.rewards.filter(r => r.type === 'badge').map(r => createBadgeFromReward(r)),
      ...
      const updatedProfile = {
        ...
        stats: {
          ...
          totalExperience: playerProfile.stats.totalExperience + experienceGained
        }
      };
      localStorage.setItem(STORAGE_KEYS.PLAYER_PROFILE, JSON.stringify(updatedProfile));
  ...
  const addExperienceMutation = useMutation({
    mutationFn: async ({ amount, source }: { amount: number; source: ExperienceSource }): Promise<LevelResult> => {
      return addExperienceInternal(amount, source);
    }
  });

  // Internal add experience function
  const addExperienceInternal = async (amount: number, source: ExperienceSource): Promise<LevelResult> => {
    ...
    const experienceToNext = newLevel < LEVEL_THRESHOLDS.length 
    ...
    localStorage.setItem(STORAGE_KEYS.EXPERIENCE_HISTORY, JSON.stringify(updatedHistory));
    queryClient.setQueryData(['gamification', 'experience', userId], updatedHistory);
    ...
    logger.info('Experience added', { amount, source, oldLevel, newLevel, userId });
  ...
  const checkLevelUp = useCallback(async (): Promise<LevelUpResult | null> => {
    const currentLevelThreshold = LEVEL_THRESHOLDS[playerProfile.level - 1] || 0;
    const nextLevelThreshold = LEVEL_THRESHOLDS[playerProfile.level] || Infinity;
  ...
  const getLeaderboardRank = useCallback(async (leaderboardId: string): Promise<LeaderboardRank> => {
    ...
    const userRank = Math.floor(Math.random() * leaderboard.totalParticipants) + 1;
    const percentile = ((leaderboard.totalParticipants - userRank) / leaderboard.totalParticipants) * 100;
  ...
  const compareWithFriends = useCallback(async (): Promise<FriendComparison[]> => {
      ...
      categories: [
        {
          ...
          status: playerProfile.level > friend.level ? 'ahead' : 
                  playerProfile.level < friend.level ? 'behind' : 'tied'
        },
        {
          category: 'Experience',
  ...
  const sendChallenge = useCallback(async (friendId: string, challengeId: string) => {
    logger.info('Challenge sent to friend', { friendId, challengeId, userId });
    // In a real implementation, this would create a challenge invitation
    await new Promise(resolve => setTimeout(resolve, 500));
  }, [userId]);

  // Get seasonal rewards
  const getSeasonalRewards = useCallback(async (): Promise<SeasonalReward[]> => {
    return currentSeason?.rewards || [];
  }, [currentSeason]);

  // Claim seasonal reward
  const claimSeasonalReward = useCallback(async (rewardId: string) => {
  ...
  const generateSeasonalRewards = (): SeasonalReward[] => [
    {
      ...
      type: 'badge',
      requirement: { type: 'challenges', value: 10, description: 'Complete 10 challenges' },
      claimed: false,
      exclusive: true
    }
  ];
  ...
  const progressToNext = playerProfile.experienceToNext > 0 
    ? ((playerProfile.experience - (LEVEL_THRESHOLDS[playerProfile.level - 1] || 0)) / 
       ((LEVEL_THRESHOLDS[playerProfile.level] || playerProfile.experienceToNext) - (LEVEL_THRESHOLDS[playerProfile.level - 1] || 0))) * 100
    : 100;
  ...
  const rank = leaderboards.length > 0 
    ? leaderboards[0].entries.findIndex(e => e.userId === userId) + 1 || 0
    : 0;

  const hasUnclaimedRewards = currentSeason?.rewards.some(r => !r.claimed) || false;
  ...
  return {
    ...
    // Errors
    error: acceptChallengeMutation.error || completeChallengeMutation.error || addExperienceMutation.error
  };
};