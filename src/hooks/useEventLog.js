...
export const useEventLog = (userId, isAuthReady) => {
  ...
  const [newEventPartnerOrgasmAmount, setNewEventPartnerOrgasmAmount] =
    useState("");
      ...
      try {
        ...
        setSexualEventsLog(
          querySnapshot.docs.map((d) => ({
            ...
            ...d.data(),
            eventTimestamp:
              d.data().eventTimestamp?.toDate() || d.data().timestamp?.toDate(),
  ...
  const handleLogNewEvent = useCallback(
    async (e) => {
      ...
      const selfOrgasmAmountNum =
        selectedEventTypes.includes("Orgasm (Self)") && newEventSelfOrgasmAmount
          ? parseInt(newEventSelfOrgasmAmount, 10) || null
          : null;
      const partnerOrgasmAmountNum =
