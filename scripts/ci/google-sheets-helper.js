...
class SheetsReporter {
  ...
  async addPerformanceData(data) {
    try {
      ...
      const rows = await sheet.getRows({ limit: 10, offset: Math.max(0, sheet.rowCount - 10) });
      const recentScores = rows.map(r => parseFloat(r.get('Overall Score'))).filter(s => !isNaN(s));
      ...
      if (recentScores.length >= 2) {
        ...
        console.log(`📊 Performance trend: ${trend > 0 ? '+' : ''}${(trend * 100).toFixed(1)}%`);
      ...
      console.error('❌ Failed to add performance data to sheets:', error.message);
  ...
  async addCodeQualityData(data) {
    try {
      ...
      const errorRate = data.totalFiles > 0 ? (data.totalErrors / data.totalFiles * 100).toFixed(2) : 0;
      const warningRate = data.totalFiles > 0 ? (data.totalWarnings / data.totalFiles * 100).toFixed(2) : 0;
      ...
      // Get quality trend
      const rows = await sheet.getRows({ limit: 10, offset: Math.max(0, sheet.rowCount - 10) });
      const recentScores = rows.map(r => parseFloat(r.get('Quality Score'))).filter(s => !isNaN(s));
      ...
      if (recentScores.length >= 2) {
        ...
        console.log(`📊 Quality trend: ${trend > 0 ? '+' : ''}${trend.toFixed(1)} points`);
      ...
      console.error('❌ Failed to add code quality data to sheets:', error.message);
    }
  }
  ...
  analyzeTrend(data, scoreColumn) {
    ...
    const scores = data.map(row => parseFloat(row.get(scoreColumn))).filter(s => !isNaN(s));
    if (scores.length < 2) {
      return { trend: 'insufficient-data', change: 0 };
    }
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log('Usage: node google-sheets-helper.js <performance|quality|report> [data]');
        process.exit(1);
    }
  } catch (error) {
