...
class DiscordReporter {
  ...
  createPerformanceEmbed(data) {
    ...
    const scoreColor = isRegression ? 15158332 : // Red for regression
                       overallScore >= 0.9 ? 3066993 : // Green for excellent
                       overallScore >= 0.8 ? 15844367 : // Yellow for good
                       15158332; // Red for poor

    const statusEmoji = isRegression ? '⚠️' :
                       overallScore >= 0.9 ? '🚀' :
                       overallScore >= 0.8 ? '✅' : '❌';
    ...
    return {
      ...
      avatarUrl: 'https://raw.githubusercontent.com/GoogleChrome/lighthouse/main/assets/lighthouse-logo.png',
      embed: {
        title,
        description,
        color: scoreColor,
  ...
  createCodeQualityEmbed(data) {
    ...
    const scoreColor = isRegression ? 15158332 : // Red for regression
                       qualityScore >= 90 ? 3066993 : // Green for excellent
                       qualityScore >= 80 ? 15844367 : // Yellow for good
                       qualityScore >= 70 ? 16753920 : // Orange for fair
                       15158332; // Red for poor
    ...
    const topRulesList = topRules?.slice(0, 3).map(([rule, count]) =>
  ...
  createWeeklyReportEmbed(trendData) {
    ...
    const getTrendColor = (perfTrend, qualTrend) => {
      if (perfTrend === 'improving' && qualTrend === 'improving') return 3066993; // Green
      if (perfTrend === 'declining' || qualTrend === 'declining') return 15158332; // Red
        ...
        fields: [
          {
            ...
            value: `**${performance.trend.toUpperCase()}**\n` +
          ...
          },
          {
            ...
            value: `**${quality.trend.toUpperCase()}**\n` +
          ...
          {
            name: '🔗 Quick Links',
            value: '[Performance Dashboard](https://sheets.google.com) • [Quality Dashboard](https://sheets.google.com) • [GitHub Issues](https://github.com/thef4tdaddy/chastityOS/issues)',
            inline: false
          }
        ],
        footer: {
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log('Usage: node discord-reporter.js <performance|quality|weekly-report> <data>');
        process.exit(1);
    }
  } catch (error) {
