...
describe("ChastityDB Database Services", () => {
  ...
  describe("SessionDBService", () => {
    ...
    it("should calculate effective duration correctly", () => {
      ...
      const effectiveDuration =
        sessionDBService.calculateEffectiveDuration(session);
  ...
  describe("EventDBService", () => {
    ...
    it("should get events by type", async () => {
      ...
      await eventDBService.logEvent(testUserId, "milestone", {
        notes: "Event 2",
      });
      await eventDBService.logEvent(testUserId, "orgasm", { notes: "Event 3" });

      const orgasmEvents = await eventDBService.getEventsByType(
        testUserId,
        "orgasm",
      );
  ...
  describe("TaskDBService", () => {
    ...
    it("should get tasks by status", async () => {
      ...
      const pendingTasks = await taskDBService.getTasksByStatus(
        testUserId,
        "pending",
      );
      const completedTasks = await taskDBService.getTasksByStatus(
        testUserId,
        "completed",
      );
    ...
    it("should identify overdue tasks", async () => {
      ...
      const overdueTaskId = await taskDBService.addTask(
        testUserId,
        "Overdue task",
        {
          dueDate: pastDate,
  ...
  describe("GoalDBService", () => {
    ...
    it("should get goals by type", async () => {
      ...
      const durationGoals = await goalDBService.getGoalsByType(
        testUserId,
        "duration",
      );
      const behavioralGoals = await goalDBService.getGoalsByType(
        testUserId,
        "behavioral",
      );
  ...
  describe("Base Database Operations", () => {
    it("should handle sync status correctly", async () => {
      ...
      const pendingItems = await sessionDBService.getPendingSync(
        testUserId + "2",
      );
    ...
    it("should handle pagination correctly", async () => {
      ...
      for (let i = 0; i < 25; i++) {
        await eventDBService.logEvent(testUserId, "note", {
          notes: `Event ${i}`,
        });
      }
    ...
    it("should handle bulk operations correctly", async () => {
      const ids: string[] = [];
      for (let i = 0; i < 5; i++) {
        const id = await eventDBService.logEvent(testUserId, "note", {
          notes: `Event ${i}`,
        });
        ids.push(id);
      }
